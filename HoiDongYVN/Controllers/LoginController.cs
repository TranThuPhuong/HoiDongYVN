...
namespace HoiDongYVN.Controllers
{
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> Login(string Username, string Password)
        {
            ...
            if (HttpContext.User.Identity.IsAuthenticated && User.IsInRole("Admin"))
            {
                // Claim đã được lưu trữ thành công
                // Tiếp tục xử lý logic cho trang Index
                return RedirectToAction("Index", "Creators");

            }
            else
            {
                // Người dùng chưa xác thực hoặc Claim không tồn tại
